...
function loss(node_values, grid_points)
    ...
    get_value_bd0(ne) = @views [0.0 ; node_values[3ne-1 : 3ne+1]]
    ...
    loss2 = 0.0
    for nedge in range(1, NE-1, step=1)
        el1 = get_element(nedge)
        el2 = get_element(nedge+1)
        loss2 = loss2 + ((D * el1.data)[end] * jacobian(el1) - (D * el2.data)[1] * jacobian(el2))^2
    end
    loss1 + loss2
end
...
function train(N::Integer)
    ...
    grid = range(0, 1, length=N+1) |> collect
    train(grid)
end
