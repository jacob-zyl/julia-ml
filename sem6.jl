...
function element_loss(el::Element, u1, u2)
    u = el.data
    j = jacobian(el)
    r = @. u * u1 - nu * u2
    sum(r.^2 .* weights) * j
...
function loss(node_values, grid_points)
    ...
    node_derivatives = zeros(NK, NE)
    node_2derivatives = zeros(NK, NE)
    ...
    element_derivatives = hcat(get_u1.(range(1, NE, step=1))...)
    # u2tmp = D * element_derivatives .* (jacobian.(elements))'
    function get_u2(n)
        if n == 1
            u2 = vec0 .* u2tmp[:, 1] + vec00 .* u2tmp[1, 2]
        elseif n == NE
            u2 = vec2 .* u2tmp[:, NE] + vec20 .* u2tmp[NK, NE-1]
        else
            u2 = vec1 .* u2tmp[:, n] + vec00 .* u2tmp[1, n+1] + vec20 .* u2tmp[NK, n-1]
        end
        u2
    end
    # element_2derivatives = hcat(get_u2.(range(1, NE, step=1))...)
    # loss = sum([element_loss(elements[n], element_derivatives[:, n], element_2derivatives[:, n]) for n in range(1, NE, step=1)])
    loss = sum([element_loss(elements[n], element_derivatives[:, n]) for n in range(1, NE, step=1)])
end
