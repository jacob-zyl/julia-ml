...
function build_model_fast()
    θ1 = initial_params(
        FastChain(FastDense(DIM, HIDDEN, tanh), FastDense(HIDDEN, 1)))
    θ2 = initial_params(
        FastChain(FastDense(DIM, HIDDEN, tanh), FastDense(HIDDEN, 1)))
    θ3 = initial_params(
        FastChain(FastDense(DIM, HIDDEN, tanh), FastDense(HIDDEN, 1)))
...
function get_loss(ϕ)
    ...
    split(phi) = (
        (t, x) -> [1.0f0, 0.0f0]' * phi(t, x),
        (t, x) -> [0.0f0, 1.0f0]' * phi(t, x))
    ...
    loss(θ, pde_domain::Array{Float32, 2}) = begin
...
function train(ϕ, Θ::Array; optimizer=BFGS(), maxiters=2000)
